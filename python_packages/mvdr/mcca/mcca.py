...
import numpy as np
from mvdr.linalg_utils import eigh_wrapper, normalize_cols, svd_wrapper
from mvdr.mcca.view_processing import center_views, initial_svds, split
...
class MCCA(BaseEstimator, TransformerMixin):
    ...
    def fit(self, Xs, precomp_svds=None):
        ...
        for b in range(n_views):
            ...
            self.views_[b] = MCCAView(view_scores=bs, view_loadings=bl, centerer=cent)
    ...
    def inverse_transform(self, Xs):
        return [self.views_[b].inverse_transform(Xs[b]) for b in range(self.n_views_)]
...
def mcca_gevp(Xs, n_components=None, center=True, regs=None):
    ...
    common_norm_scores, view_scores, view_loadings = mcca_det_output(
...
def check_regs(regs, n_views):
    ...
    if regs is not None:
        assert len(regs) == n_views, "regs should be None or len(regs) == n_views"
...
def flag_mean(bases, n_components=None, weights=None):
    ...
    """
    Xs, n_views, ambient_dim, subspace_dims = check_Xs(
    ...
    flag_mean, svals, view_loadings = svd_wrapper(np.block(bases), rank=n_components)

    # get the view loadings and scores
    view_loadings = split(view_loadings, dims=subspace_dims, axis=0)
    view_scores = [bases[b] @ view_loadings[b] for b in range(n_views)]

    flag_mean, view_scores, view_loadings = mcca_det_output(
