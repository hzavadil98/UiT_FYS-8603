...
def sample_part_shared_fact_model(
    ranks,
    svals,
    n_samples=200,
    n_features=[10, 20, 30],
    ...
    # sample view errors
    errors = [
        noise_std[b] * rng.normal(size=(n_samples, n_features[b]))
        for b in range(n_views)
    ]
    ...
    for S in ranks.keys():
        K = ranks[S]
        for b in S:
            view_loadings[b][S] = rand_orthog(n=n_features[b], K=K, random_state=rng)

    # setup view signal matices
    view_signal_mats = [np.zeros((n_samples, n_features[b])) for b in range(n_views)]
    for S in ranks.keys():
        for b in S:
            W = view_loadings[b][S]
            sv = svals[b][S]
...
def get_part_shared_struct_ranks(n_views=3, rank=2, min_size=1, max_size=None):
    """
...
def scale_svals(svals, n_samples, n_features, noise_std, m=1.5):
    ...
    for b in range(n_views):
        ...
        base = m * noise_std[b] * (n_samples * n_features[b]) ** (0.25)
...
def get_scaled_ps_sval_spikes(
    ranks, n_samples, n_features, noise_std, m=1.5, random_state=None
):
    """
    ...
    svals = get_ps_sval_spikes(
        n_views=len(n_features), ranks=ranks, random_state=random_state
    )
