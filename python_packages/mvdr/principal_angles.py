...
def subspace_dist(A, B, method="proj_f", is_ortho=True):
    ...
    if method not in [
        ...
        raise ValueError(" method = {} is not currently an option".format(method))
    ...
    if method == "arc_length":
        ...
        return max(2 * np.sin(0.5 * theta))

    elif method == "chordal_f":
        return np.linalg.norm(2 * np.sin(0.5 * theta))

    if method == "proj_f":
        diff = A @ A.T - B @ B.T
        return np.sqrt(0.5) * np.linalg.norm(diff, ord="fro")
