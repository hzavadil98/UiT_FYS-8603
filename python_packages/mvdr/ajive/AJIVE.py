...
from mvdr.ajive.ajive_fun import _ajive_docs, ajive
...
class AJIVE(BaseEstimator):
    ...
    def fit(self, Xs):
        ...
        usr_iniv_ranks = arg_checker(Xs=Xs, usr_iniv_ranks=self.usr_iniv_ranks)

        assert (
            self.init_signal_ranks is not None
            and len(self.init_signal_ranks) == n_views
        )
    ...
    def get_ranks(self):
        ...
        return self.common_.n_components, [
            self.view_specific_[b].indiv_rank_ for b in range(self.n_views_)
        ]
    ...
    def get_view_decomps(self):
        ...
        for b in range(self.n_views_):
            full[b] = {
                "joint": self.view_specific_[b].joint_.full_,
    ...
    def plot_joint_diagnostic(self, fontsize=20):
        ...
        plot_joint_diagnostic(
            all_common_svals=all_common_svals,
...
def get_mcca_from_ajive_out(ajive_out):
    ...
    for b in range(n_views):
        ...
        views[b] = MCCAView(view_scores=bs, view_loadings=bl, centerer=cent)
...
class ViewSpecificResults(object):
    ...
    def __init__(self, decomps, centerer, sv_threshold=None, view_idx=None):
        ...
        if decomps["individual"]["scores"] is None:
            ...
            self.individual_ = get_pca(decomps["individual"], centerer=centerer)
    ...
    def __repr__(self):
        return "View: {}, individual rank: {}, joint rank: {}".format(
            self.view_idx_, self.indiv_rank_, self.joint_rank_
        )
