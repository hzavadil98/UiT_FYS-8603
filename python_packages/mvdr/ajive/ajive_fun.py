...
from mvdr.mcca.view_processing import center_views, split
...
def ajive(
    ...
    indiv_ranks=None,
    center=True,
    common_loading_method="map_back",
    ...
    Xs, init_signal_ranks, indiv_ranks = arg_checker(
        Xs,
        init_signal_ranks,
        joint_rank,
        indiv_ranks,
    ...
    for b in range(n_views):
        ...
        sv_thresholds[b] = t
        init_signal_svd[b] = {
            "scores": U[:, 0 : init_signal_ranks[b]],
    ...
    if joint_rank is None:  # estimate the joint rank
        joint_rank, rank_est_data = estimate_joint_rank(
            Xs=Xs,
    ...
    if joint_rank == 0:
        ...
        if check_joint_identif:
            common_scores, common_svals, common_loadings, joint_rank, dropped = (
                check_identifiability(
                    Xs, sv_thresholds, common_scores, common_svals, common_loadings
                )
            )
        ...
        ############################################
        common_loadings = split(common_loadings, dims=init_signal_ranks, axis=0)
        view_loadings = [None for b in range(n_views)]
        if common_loading_method == "map_back":
            ...
            for b in range(n_views):
                # V D^{-1} W
                view_loadings[b] = np.multiply(
                    init_signal_svd[b]["loadings"], 1.0 / init_signal_svd[b]["svals"]
            ...
            for b in range(n_views):
                ...
                for r in range(joint_rank):
                    ...
                    view_loadings[b][:, r] = get_coef(X=Xs[b], y=common_scores[r])
...
def arg_checker(
    ...
    if len(set(Xs[b].shape[0] for b in range(n_views))) != 1:
        raise ValueError("Blocks must have same number of observations (rows).")
    ...
    for b in range(n_views):
        ...
        if not (1 <= r) or not (r <= min(n_samples, n_features[b]) - 1):
            raise ValueError(
                "initial signal rank for view {} must be "
    ...
    if joint_rank is not None and joint_rank > sum(init_signal_ranks):
        raise ValueError(
            "joint_rank must be smaller than the sumof the initial signal ranks"
        )
    if joint_rank is not None:
        if sum([n_wedin_samples is None, n_rand_samples is None]) == 2:
            raise ValueError(
                "If joint_rank is not provided, at least one"
    ...
    if len(indiv_ranks) != n_views:
        raise ValueError(
            "If individual signal ranks are provided,"
    ...
    for b in range(n_views):
        ...
        if not (r is None or isinstance(r, numbers.Number)):
            raise ValueError(
                "Individual signal rank provided for view {} "
    ...
    if not callable(common_loading_method):
        if common_loading_method not in ["map_back", "regress"]:
            raise ValueError(
                "common_loading_method must be one of"
                "['map_back', 'regress'], not {}".format(common_loading_method)
            )

        if common_loading_method == "regress":
            if max(n_features) >= n_samples:
                raise ValueError(
                    "common_loading_method = regress is only"
...
def estimate_joint_rank(
    ...
    score_dims = [init_signal_svd[b]["scores"].shape[1] for b in range(n_views)]
    ...
    if n_rand_samples is not None:
        rand_sv_samples = sample_randdir(
            n=n_obs,
            dims=score_dims,
    ...
    if n_wedin_samples is not None:
        for b in range(n_views):
            view_wedin_samples[b] = get_wedin_samples(
                X=Xs[b],
        ...
        wedin_sv_samples = len(Xs) - np.array(
            [
                sum(view_wedin_samples[b][r] ** 2 for b in range(n_views))
    ...
    return joint_rank, {
        "svalsq_threshold": svalsq_threshold,
        "rand": {"samples": rand_sv_samples, "threshold": rand_threshold},
        "wedin": {
            "view_samples": view_wedin_samples,
