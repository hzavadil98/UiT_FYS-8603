...
def rand_2block_fig2(n=100, dx=200, dy=10000, scale_x=5000, seed=None):
    ...
    X_joint = (
        np.hstack(
            [
                np.vstack(
                    [np.ones((n // 2, dx // 2)), -1 * np.ones((n // 2, dx // 2))]
                ),
                np.zeros((n, dx // 2)),
            ]
        )
    ...
    Y_joint = np.hstack(
        [
            np.zeros((n, 4 * dy // 5)),
            np.vstack([-1 * np.ones((n // 2, dy // 5)), np.ones((n // 2, dy // 5))]),
        ]
    )
    ...
    return {
        "x": {"obs": X, "joint": X_joint, "indiv": X_indiv, "noise": X_noise},
        "y": {"obs": Y, "joint": Y_joint, "indiv": Y_indiv, "noise": Y_noise},
    }
...
def plot_jive_residuals_2block(data, estimates):
    ...
    # compute residuals
    epsilon = 1e-8
    R_joint_x = threshold(data["x"]["joint"] - estimates["x"]["joint"], epsilon)
    R_indiv_x = threshold(data["x"]["indiv"] - estimates["x"]["indiv"], epsilon)
    R_noise_x = threshold(data["x"]["noise"] - estimates["x"]["noise"], epsilon)

    R_joint_y = threshold(data["y"]["joint"] - estimates["y"]["joint"], epsilon)
    R_indiv_y = threshold(data["y"]["indiv"] - estimates["y"]["indiv"], epsilon)
    R_noise_y = threshold(data["y"]["noise"] - estimates["y"]["noise"], epsilon)
...
def show_heatmap(A):
    ...
    sns.heatmap(A, xticklabels=d // 5, yticklabels=n // 5, cmap="RdBu")
