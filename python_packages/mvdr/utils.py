...
def draw_samples(
    fun, n_draws=1, n_jobs=None, backend=None, random_state=None, args=[], kws={}
):
    """
    ...
    if n_jobs is not None:
        return Parallel(n_jobs=n_jobs, backend=backend)(
            delayed(fun)(random_state=seeds[d], *args, **kws) for d in range(n_draws)
        )

    else:
        return [fun(random_state=seeds[s], *args, **kws) for s in range(n_draws)]
