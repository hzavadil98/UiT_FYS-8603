...
class TestLinearOperator(object):
    ...
    def test_transpose_basic(self):
        ...
        assert_equal(
            A.T * np.array([[1, 2], [3, 4]]),  # multiply by matrix
...
class TestAsLinearOperator(object):
    ...
    def setUp(self):
        ...
        def make_cases(dtype):
            ...
            def rmv(x, dtype):
                return np.array(
                    [1 * x[0] + 4 * x[1], 2 * x[0] + 5 * x[1], 3 * x[0] + 6 * x[1]],
                    dtype=dtype,
                )
    ...
    def test_basic(self):
        for M in self.cases:
            ...
            assert_equal(
                A.matmat(np.array([[1, 4], [2, 5], [3, 6]])), [[14, 32], [32, 77]]
            )

            assert_equal(
                A.T.matmat(np.array([[1, 2], [3, 4]])), [[13, 18], [17, 24], [21, 30]]
            )
    ...
    def test_dot(self):
        for M in self.cases:
            ...
            assert_equal(
                A.dot(np.array([[1, 4], [2, 5], [3, 6]])), [[14, 32], [32, 77]]
            )

            assert_equal(
                A.T.dot(np.array([[1, 2], [3, 4]])), [[13, 18], [17, 24], [21, 30]]
            )
