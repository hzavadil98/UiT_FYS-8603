import numpy as np
from jive.lazymatpy.interface import IdentityOperator, LinearOperator, aslinearoperator
...
def col_mean_centered(S):
    ...
    return aslinearoperator(S) - OnesOuterVec(num_ones=S.shape[0], vec=m)
...
def col_proj_orthog(M, U):
    """(I - UU^T)M"""
    return (
        IdentityOperator(shape=(M.shape[0], M.shape[0]), dtype=M.dtype)
        - aslinearoperator(U) * aslinearoperator(U.T)
    ) * aslinearoperator(M)
