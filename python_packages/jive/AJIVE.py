...
from jive.utils import centering, svd_wrapper
    ...
    def fit(self, blocks, precomp_init_svd=None):
        ...
        for bn in block_names:
            ...
            self.sv_threshold_[bn] = (
                D[self.init_signal_ranks[bn] - 1] + D[self.init_signal_ranks[bn]]
            ) / 2
        ...
        self.common.set_comp_names(base="common", zero_index=self.zero_index_names)
...
def arg_checker(
    blocks, init_signal_ranks, joint_rank, indiv_ranks, precomp_init_svd, center
):
    """ """
    # TODO: document
...
class BlockSpecificResults(object):
    ...
    def __init__(
        ...
        if individual["rank"] != 0:
            ...
            if block_name is not None:
                base = "{}_{}".format(block_name, base)
            self.individual.set_comp_names(base=base, zero_index=zero_index_names)
        ...
        if noise is not None:
            self.noise_ = pd.DataFrame(noise, index=obs_names, columns=var_names)
        else:
            self.noise_ = None
        ...
        U, D, V = (
            init_signal_svd["scores"],
            init_signal_svd["svals"],
        ...
        comp_names = get_comp_names(
            base=base, num=CNS.shape[1], zero_index=zero_index_names
        )
        self.common_loadings_ = pd.DataFrame(
            common_loadigs, index=var_names, columns=comp_names
        )
        ...
        self.common_loadings_reg_X = pd.DataFrame(
            common_loadings_reg_X, index=var_names, columns=comp_names
        )
    ...
    def plot_common_loading(
        ...
        """
        plot_loading(
            v=self.common_loadings_.iloc[:, comp],
            abs_sorted=abs_sorted,
            show_var_names=show_var_names,
