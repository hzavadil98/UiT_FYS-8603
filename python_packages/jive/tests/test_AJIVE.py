...
class TestFig2Runs(unittest.TestCase):
    ...
    def test_matrix_decomposition(self):
        ...
        Rx = X_cent - (
            self.ajive.blocks["x"].joint.full_
        ...
        Ry = Y_cent - (
            self.ajive.blocks["y"].joint.full_
    ...
    def test_names(self):
        self.assertEqual(set(self.ajive.common.obs_names()), set(self.obs_names))
        self.assertEqual(set(self.ajive.common.scores_.index), set(self.obs_names))

        self.assertEqual(
            set(self.ajive.blocks["x"].joint.obs_names()), set(self.obs_names)
        )

        self.assertEqual(
            set(self.ajive.blocks["x"].joint.scores_.index), set(self.obs_names)
        )

        self.assertEqual(
            set(self.ajive.blocks["x"].joint.var_names()), set(self.var_names["x"])
        )

        self.assertEqual(
            set(self.ajive.blocks["x"].individual.obs_names()), set(self.obs_names)
        )

        self.assertEqual(
            set(self.ajive.blocks["x"].individual.var_names()), set(self.var_names["x"])
        )
    ...
    def test_centering(self):
        ...
        # only center x
        ajive = AJIVE(
            init_signal_ranks={"x": 2, "y": 3}, center={"x": True, "y": False}
        )
        ajive = ajive.fit(blocks={"x": self.X, "y": self.Y})
