...
def plot_joint_diagnostic(
    ...
    plt.axvline(wedin_cutoff, color="blue", ls="dashed", lw=wedin_lw, label=wedin_label)

    # random cutoff
    plt.axvspan(rand_low, rand_high, alpha=0.1, color="red")
    plt.axvline(rand_cutoff, color="red", ls="dashed", lw=rand_lw, label=rand_label)
    ...
    for i, sv in enumerate(svals):
        ...
        plt.axvline(
            sv_sq, ymin=0.05, ymax=0.95, color=color, label=label, lw=2, zorder=2
        )
    ...
    plt.xlim(1)
    plt.title(
        "joint singular value thresholding (joint rank estimate = {:d})".format(
            joint_rank_est
        ),
