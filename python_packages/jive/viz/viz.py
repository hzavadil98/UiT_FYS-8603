...
from jive.viz.scatter_plot_save_selected import (
    get_handle,
    get_save_selected,
    setup_data,
)
...
def jitter_hist(x, **kwargs):
    ...
    y = np.random.uniform(low=0.05 * max(n), high=0.1 * max(n), size=len(x))
...
def interactive_slice(x, y, cats=None, obs_names=None, xlab="x", ylab="y"):
    ...
    if obs_names is None:
        obs_names = list(range(len(x)))

    source, model = setup_data(x=x, y=y, names=obs_names, classes=cats)
