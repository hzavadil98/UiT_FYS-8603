...
def generate_data_ajive_fig2(seed=None, all_components=False):
    ...
    # Sample X data
    X_joint = np.bmat([[np.ones((50, 50))], [-1 * np.ones((50, 50))]])
    X_joint = 5000 * np.bmat([X_joint, np.zeros((100, 50))])

    X_indiv = 5000 * np.bmat(
        [
            [-1 * np.ones((25, 100))],
            [np.ones((25, 100))],
            [-1 * np.ones((25, 100))],
    ...
    # Sample Y data
    Y_joint = np.bmat([[-1 * np.ones((50, 2000))], [np.ones((50, 2000))]])
    ...
    Y_indiv_b = np.bmat(
        [[np.ones((25, 5000))], [-1 * np.ones((50, 5000))], [np.ones((25, 5000))]]
    )
...
def plot_ajive_fig2(X_obs, X_joint, X_indiv, X_noise, Y_obs, Y_joint, Y_indiv, Y_noise):
    """
    ...
    # Observed
    plt.subplot(4, 2, 1)
    sns.heatmap(np_matrix_to_pd_dataframe(X_obs), xticklabels=20, yticklabels=20)
    plt.title("X observed")

    plt.subplot(4, 2, 2)
    sns.heatmap(np_matrix_to_pd_dataframe(Y_obs), xticklabels=2000, yticklabels=20)
...
def plot_jive_results_2blocks(X_obs, J_x, I_x, E_x, Y_obs, J_y, I_y, E_y):
    """
...
def plot_jive_residuals_2block(
    X_joint, X_indiv, X_noise, J_x, I_x, E_x, Y_joint, Y_indiv, Y_noise, J_y, I_y, E_y
):
