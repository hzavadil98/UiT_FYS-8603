...
from jive.viz.singular_values import plot_var_expl_cum, plot_var_expl_prop, scree_plot
from jive.viz.viz import interactive_slice, plot_loading, plot_scores_hist
...
class PCA(object):
    ...
    def get_params(self):
        return {"n_components": self.n_components, "center": self.center}
    ...
    def fit(self, X):
        ...
        self.scores_, self.svals_, self.loadings_ = svd2pd(
            U, D, V, obs_names=obs_names, var_names=var_names
        )
    ...
    def from_precomputed(
        ...
        if scores is not None and type(scores) != pd.DataFrame:
            ...
            scores = pd.DataFrame(scores, index=obs_names, columns=comp_names)
        ...
        if loadings is not None and type(loadings) != pd.DataFrame:
            ...
            if comp_names is None:
                comp_names = _default_comp_names(loadings.shape[1])
            loadings = pd.DataFrame(loadings, index=var_names, columns=comp_names)
        x.scores_ = scores
    ...
    def set_comp_names(self, comp_names=None, base=None, zero_index=True):
        ...
        if comp_names is None:
            comp_names = get_comp_names(
                base=base, num=len(self.scores_.columns), zero_index=zero_index
            )
    ...
    def scores_corr_vs(self, y):
        ...
        return np.array(
            [
                np.corrcoef(self.scores().iloc[:, i], y)[0, 1]
