...
class PCA(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X):
        ...
        self.centerer_ = StandardScaler(
            copy=True, with_mean=self.center, with_std=False
        )
        ...
        if isinstance(self.n_components, str):
            start_time = time()
            UDV, rank_est, out = select_rank(
                X=X, method=self.n_components, UDV=UDV, **self.rank_sel_kws
            )
            ...
            if rank_est == rank2compute:
                warn(
                    "The rank selection picked the largest possible"
                    " rank of {}, which may indicate an issue with"
                    " the rank selection method. It may be a good idea"
                    " to look at diagnostics for the rank selection method.".format(
                        rank_est
                    )
                )
    ...
    def score_samples(self, X):
        if self.score_mode == "euclid":
            ...
            raise ValueError(
                "Invalid inpute for score_mode: {}.Must be one of {}.".format(valid)
            )
