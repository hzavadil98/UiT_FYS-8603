...
from ya_pca.rank_selection.variance_explained import select_rank_variance_explained
...
def select_rank(
    X, method="rmt_threshold", UDV=None, max_rank=None, random_state=None, **kwargs
):
    """
    ...
    **kwargs:
    ...
    if method == "rmt_threshold":
        ...
        rank_est, out = select_rank_bi_cv(
            X=X, max_rank=max_rank, random_state=random_state, **kwargs
        )

    elif method == "wold_cv":
        rank_est, out = select_rank_wold_cv(
            X=X, max_rank=max_rank, random_state=random_state, **kwargs
        )

    elif method == "bai_ng_bic":
        rank_est, out = select_rank_bai_ng_bic(
            X=X, UDV=UDV, max_rank=max_rank, **kwargs
        )

    elif method == "horn":
        rank_est, out = select_rank_horn(
            X=X, svals=svals, max_rank=max_rank, random_state=random_state
        )
        ...
        rank_est, out = select_rank_variance_explained(X=X, svals=svals, **kwargs)

    else:
        raise ValueError(
            "Invalid input for method: {}. Must be one of {}.".format(
