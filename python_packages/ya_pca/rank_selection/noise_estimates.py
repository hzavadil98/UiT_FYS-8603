    ...
    **kwargs:
    ...
    if method.lower() == "mp":
        if UDV is None:
            UDV = svd_wrapper(X)
        svals = UDV[1]
        return estimate_noise_mp_quantile(svals=svals, shape=X.shape, **kwargs)
...
def estimate_noise_mp_quantile(svals, shape, q=0.5):
    ...
    if len(svals) < min(shape):
        ...
        if new_q > q:
            warn(
                "Only {}/{} singular values provided, but asked for the "
                "{}th quantile. Falling back on the {}th quantile/".format(
                    len(svals), min(shape), q, new_q
                )
            )
        ...
        svals = np.concatenate([svals, np.repeat(min(svals), repeats=n_to_add)])
        # TODO: test this!
...
def estimate_noise_soft_impute(
    X, UDV=None, c=2 / 3, K=5, n_lambd_vals=100, max_rank=None, max_rank_si=None, **kws
):
    """
    ...
    lambd_cv, out = soft_impute_cv(
        X, K=K, n_lambd_vals=n_lambd_vals, max_rank=max_rank_si, **kws
    )
    ...
    m = min(X.shape)
    _svals = np.concatenate([svals, np.zeros(m - n_svals_computed)])

    ndof = (_svals > lambd_cv).sum()

    if ndof >= n_svals_computed:  # ge in case lambd_cv == 0
        warn(
            "max_rank may be too small and have affected soft_imputs's noise estimate."
        )
