...
def select_rank_wold_cv(
    X, max_rank, p_holdout=0.3, opt_kws={}, n_folds=5, random_state=None, rotate=False
):
    """
    ...
    for fold in range(n_folds):
        ...
        for rank_idx, rank in enumerate(ranks):
            out = svd_missing(X, M=M, rank=rank, random_state=rng, **opt_kws)
    ...
    return rank_est, {"fold_errors": fold_errors, "errors": errors}
...
def plot_wold_cv(errors):
    ...
    if errors.index.name == "rank":
        rank_est = errors.idxmin()
        plt.axvline(rank_est, label="Rank est: {}".format(rank_est), color="red")
