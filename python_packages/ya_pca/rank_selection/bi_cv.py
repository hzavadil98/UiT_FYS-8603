...
from ya_pca.linalg_utils import rand_orthog, svd_wrapper


def select_rank_bi_cv(
    X, max_rank=None, krow=2, kcol=2, random_state=None, rotate=False
):
    """
    ...
    bicv_splitter = BiCvFolds(
        shape=X.shape, krow=krow, kcol=kcol, random_state=random_state
    )
    ...
    for (
        ...
        test_rows,
        test_cols,
    ) in bicv_splitter.iter_matrix(X):
        # compute SVD of training block
        UDV_tr = svd_wrapper(X_tr, rank=max_rank)

        # compute error or each test block
        for rank_idx, rank in enumerate(ranks):
            grid_errors[row_idx, col_idx, rank_idx] = get_error(
                UDV_tr=UDV_tr, rank=rank, Y_tr=Y_tr, X_tst=X_tst, Y_tst=Y_tst
            )
    ...
    if rank_est == max_rank:
        warn(
            "Bi-CV picked the largest possible rank of {}, which suggests "
            "we did not examine a enough rank. Try increasing krow/kcol.".format(
                rank_est
            )
        )
...
class BiCvFolds(object):
    ...
    def __init__(self, shape, krow=2, kcol=2, random_state=None):
        ...
        self.shape = shape
        self.row_splitter = KFold(n_splits=krow, shuffle=True, random_state=rng)
        self.col_splitter = KFold(n_splits=kcol, shuffle=True, random_state=rng)
    ...
    def max_rank_(self):
        ...
        return int(
            np.floor(min(n_rows - (n_rows / self.krow), n_cols - (n_cols / self.kcol)))
        )
    ...
    def split(self):
        ...
        for row_idx, (train_rows, test_rows) in enumerate(row_folds):
            for col_idx, (train_cols, test_cols) in enumerate(col_folds):
                yield row_idx, train_rows, test_rows, col_idx, train_cols, test_cols
    ...
    def iter_matrix(self, X):
        for (
            ...
            yield X_tr, Y_tr, X_tst, Y_tst, row_idx, col_idx, test_rows, test_cols
...
def get_error(UDV_tr, rank, Y_tr, X_tst, Y_tst):
    ...
    # training PCA scores
    U_tr, D_tr, V_tr = UDV_tr[0][:, 0:rank], UDV_tr[1][0:rank], UDV_tr[2][:, 0:rank]
