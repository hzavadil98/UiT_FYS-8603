...
def select_rank_rmt_threshold(
    X, thresh_method="dg", noise_est="mp", noise_est_kwargs={}, UDV=None
):
    """
    ...
    if not isinstance(noise_est, Number):
        noise_est = estimate_noise(X, method=noise_est, UDV=UDV, **noise_est_kwargs)

    assert thresh_method in ["mpe", "dg"]

    if thresh_method == "dg":
        thresh = donoho_gavish_threshold(shape=X.shape, sigma=noise_est)

    elif thresh_method == "mpe":
        thresh = marcenko_pastur_edge_threshold(shape=X.shape, sigma=noise_est)
