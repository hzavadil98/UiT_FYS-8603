...
def soft_impute_cv(
    ...
    array_indices = np.array(
        [(i, j) for (i, j) in product(range(X.shape[0]), range(X.shape[1]))]
    )
    ...
    for fold_idx, (tr_idxs, tst_idxs) in enumerate(splitter.split(index_indices)):
        # setup missing mask
        ...
        if n_jobs is not None:
            MSEs = Parallel(n_jobs=n_jobs, backend=backend)(
                delayed(fun)(lambd_val) for lambd_val in lambd_vals
            )
