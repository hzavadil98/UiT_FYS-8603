...
def rand_factor_model(
    n_samples=100, n_features=20, rank=5, m=1.5, noise_std=1.0, random_state=None
):
    """
    ...
    return X, {"U": U, "svals": svals, "V": V, "E": E}
...
def lin_spaced_svals(n_samples, n_features, rank, sigma_sq=1, m=1.5):
    ...
    sval_base = m * np.sqrt(sigma_sq) * (n_samples * n_features) ** (0.25)
    svals = np.arange(1, rank + 1) * sval_base
    svals = np.sort(svals)[::-1]
    return svals


def perry_sim_dist(strong=True, sparse=False, noise="white", random_state=None):
    """
