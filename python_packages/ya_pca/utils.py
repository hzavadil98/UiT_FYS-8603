...
def sample_parallel(
    fun, kws={}, n_samples=100, random_state=None, n_jobs=None, backend=None
):
    """
    ...
    if n_jobs is not None:
        return Parallel(n_jobs=n_jobs, backend=backend)(
            delayed(fun)(random_state=seeds[s], **kws) for s in range(n_samples)
        )

    else:
        return [fun(random_state=seeds[s], **kws) for s in range(n_samples)]
