...
def generate_mcca_test_settings():
    ...
    yield {"n_components": None, "regs": 0.1, "center": True}
    yield {"n_components": 5, "regs": 0.1, "center": True}
...
def check_mcca_scores_and_loadings(
    ...
    if check_normalization:
        ...
        if regs is None:
            assert np.allclose(
                common_norm_scores.T @ common_norm_scores,
...
def check_mcca_class(mcca, Xs):
    ...
    for b in range(mcca.n_views_):
        assert np.allclose(mcca.views_[b].view_scores_, mcca.views_[b].transform(Xs[b]))
