...
from elements.shapes import circle, cross, plus, square, triangle
...
class Element(Dataset):
    def __init__(
        self, size, shape, color, texture=None, color_seed=None, texture_seed=None
    ):
        self.size = size
        self.shape = shape
        self.color = color
        ...
        self.img = (
            self.img.reshape((size, size, 1)) * (self.color_value * colors[color])
        ).astype(np.uint8)
    ...
    def __repr__(self):
        return (
            "Element <" + ", ".join([f"{k}: {v}" for k, v in self.config.items()]) + ">"
        )
...
class ElementImage:
    def __init__(
        ...
        self.locs = self.choose_locations(
            size,
            elements,
            loc_seed,
    ...
    def place_element(canvas, element, loc):
        if loc is not None:
            canvas[loc[0] : loc[0] + len(element), loc[1] : loc[1] + len(element)] = (
                element.img
            )
        return canvas
    ...
    def choose_locations(
        ...
        for element_size in element_sizes:
            ...
            while (not placed) and (attempts < max_attempts):
                ...
                for i, coord in enumerate(locations):
                    if coord is not None:
                        overlap = check_overlap(
                            x, y, element_size, element_sizes[i], coord
                        )
                    if overlap:
                        break
                if overlap:
                ...
                if loc_restrictions is not None:
                    ...
                    for loc_restriction in loc_restrictions:
                        failed_restriction = ElementImage.check_fail_loc_restriction(
                            loc_restriction, x, y, element_size
                        )
            ...
            if not placed:
                if raise_error:
                    raise ValueError(
                        "Not all elements could be placed without overlapping!"
                    )
                    ...
                    for i in range(remaining_elements):
                        x, y = rng.integers(
                            0, canvas_size - element_sizes[len(locations)], 2
                        )
                        locations.append((x, y))
                    break
                else:
...
class ElementDataset:
        ...
        if self.allowed_combinations is not None:
            if type(self.allowed_combinations[0]) is list:
                self.allowed_combinations = [
                    tuple(v) for v in self.allowed_combinations
                ]
        self.loc_restrictions = loc_restrictions
        self.place_remaining_randomly = place_remaining_randomly
    ...
    def choose_element_configs(
        ...
        while len(configs) < n:
            ...
            if i > max_iter:
                raise TimeoutError(
                    f"Only {len(configs)} out of {n} chosen "
...
class GroupedElementDataset(ElementDataset):
    def __init__(self, **kwargs):
        ...
        self.idx_array = [
            np.where(all_class_labels[:, i])[0]
            for i in range(all_class_labels.shape[1])
        ]
...
class ConceptElementDatasetCreator:
    ...
    def __init__(
        self, allowed, class_configs, dataset_kwargs, allowed_combinations=None
    ):
        self.allowed = allowed
        self.class_configs = class_configs
        self.allowed_combinations = allowed_combinations
        ...
        self.class_dataset = GroupedElementDataset(
            allowed=allowed,
